...
# 加入交易成本（每次换仓0.1%手续费）
trades = pred_bt.diff().abs().fillna(0)
transaction_cost = trades * 0.001
strategy_returns_net = strategy_returns - transaction_cost

cum_strategy_net = (1 + strategy_returns_net).cumprod()

# 最大回撤计算
running_max = cum_strategy_net.cummax()
drawdown = (cum_strategy_net - running_max) / running_max
...
col2.metric("策略累计收益(含成本)", f"{cum_strategy_net.iloc[-1]:.2f}x")
col3.metric("最大回撤", f"{max_drawdown:.1%}")
col4.metric("持仓月数", f"{pred_bt.sum()} / {len(pred_bt)}")